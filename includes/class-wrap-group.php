...
class WrapGroup {
	public static function init() {
		...
		add_filter( 'manage_edit-wrap-group_columns', array( self::class, 'add_group_columns' ) );
		add_action( 'manage_wrap-group_custom_column', array( self::class, 'fill_group_columns' ), 10, 3 );
	}
	...
	public static function add_admin_menu() {
		add_submenu_page( 'wrap', __( 'Groups', 'wrap' ), __( 'Groups', 'wrap' ), 'manage_options', 'edit-tags.php?taxonomy=wrap-group' );
	}
	...
	public static function fill_group_columns( $content, $column_name, $term_id ) {
		if ( $column_name === 'group_users' ) {
			...
			if ( ! empty( $group_users ) && is_array( $group_users ) ) {
				$users = get_users( array( 'include' => $group_users ) );
				if ( $users ) {
